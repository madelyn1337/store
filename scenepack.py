...
class VideoProcessor:
    ...
    def set_preset(self):
        try:
            ...
            # Create complete PowerShell command with proper escaping
            ffmpeg_command = f'powershell.exe -c "Get-ChildItem \\"C:\\DMFS\\virtual\\*.avi\\" | ForEach-Object {{ {ffmpeg_base} \\"\\"\\"{output_path}\\"\\"\\" }}"'
            
            # Escape the command for PowerShell registry setting
            escaped_command = ffmpeg_command.replace('"', '`"')
            
            # Set registry values with escaped command
            ps_script = f'''
            ...
            Set-ItemProperty -Path "HKCU:\\Software\\DebugMode\\FrameServer" -Name "commandToRunOnFsStart" -Value "{escaped_command}"
            '''
